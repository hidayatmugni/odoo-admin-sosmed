...
class ContentReportStreamTiktok(models.Model):
    _name = 'content.report.stream.tiktok'
    _description = 'Report Live Streaming Tiktok'
        ...
        for rec in self:
            if rec.report_id and rec.date:
                if not (rec.report_id.date_from <= rec.date <= rec.report_id.date_to):
                    raise ValidationError("Tanggal Tiktok Stream di luar range laporan mingguan!")
